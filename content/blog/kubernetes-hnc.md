...
qui ne se font pas forcement confiance et doivent et/ou peuvent avoir des
droits et des permissions différentes. Ces tiers sont appelés des *tenants*. Ce
terme est également repris par différents fournisseurs de Cloud, notamment
OpenStack qui appelaient ses projets tenants avant de les renommer *Projects*,
c'est une notion également reprise chez Google Cloud ou Azure avec les
*Projects* et les *Resources Groups* respectivement.

Il est possible de déployer de multiples clusters Kubernetes au sein d'une
...
dont le but et de faciliter la création de *Tenant* sur un cluster via
...
dont le but est de séparer les clusters Kubernetes en de multiples sous
clusters pour une utilisation plus hermétique
* [Hierarchical namespaces controller (aka HNC)](https://github.com/kubernetes-sigs/multi-tenancy/blob/master/incubator/hnc)
...
problématique que nous rencontrons souvent, à savoir, comment donner des
...
* Les ressources *cluster non namespacés* comme les nodes et les namespaces
...
Imaginons maintenant un cas pratique où l'administration des clusters serait
gérée par une équipe. Et le déploiement des applications métier par une autre.

Il n'est pas souhaitable que l'équipe responsable des applications soit
administratice du cluster. Si cette même équipe opte pour un déploiement des
micro services par namespace (un namespace = un micro-service), cela signifie une
interaction avec l'équipe responsable de la gestion des clusters afin de créer au
préalable les namespaces et déléguer les droits à l'équipe applicative.

Dans un soucis d'automatisation permanent, cette solution n'est pas viable,
nous souhaitons que les équipes puissent être administratrices de leur propre
...
et permettent de répondre à cette problématique en organisant les namespaces sous
...
* Une CLI intégrée à `kubectl`.

### Déploiement du contrôleur

Dans un premier temps le contrôleur doit être déployé sur le cluster :
...
pour `kubectl` et HNC dispose d'un plugin prêt à l'emploi, une fois le
contrôleur installé dans le cluster, le plugin doit être intégré localement à
`kubectl` :
...
Pour le moment nous n'avons aucun sous namespace.
...
ressources namespacées qui appartiennent au namespace parent.
...
namespace donné tout en empêchant les utilisateurs de toucher à d'autres
namespaces.

La force des namespaces hiérarchiques réside également dans la réplication de
certaines ressources par défaut, notamment les *Roles* et *RoleBindings*. Ces
dernières sont automatiquement répliquées dans les sous namespaces.

Cela permet de donner uniquement des droits d'administrateur à la team A dans le
namespace `team-a` et ses droits seront automatiquement répliqués dans les sous
namespaces.
...
mais il est possible de repliquer par exemple les *Secrets* en changeant la
...
Cette configuration propagera automatiquement les secrets d'un namespace
parent vers un namespace enfant.

### Intégration avec GitOps

Les commandes vues précedemment se basent sur la CLI `kubectl` mais en realité la CLI ne
...
Il est possible de donner par exemple un accès à FluxCD à une équipe dans un
namespace donné avec le workflow suivant :

1. Création d'un namespace racine dans lequel une équipe sera administratrice.
2. Déploiement de Flux avec des droits limités uniquement dans ce namespace.
